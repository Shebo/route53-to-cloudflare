...
def set_ZoneName(zone):
    ...
    if zone['Name'].endswith('.'): # BUG - no more '.' after replacing in previous line
        zoneName=zoneName[0:-1]

    return zoneName

def set_ResourceName(name):
    ...
    #     name = '_{}'.format(name)
    name = '_{}'.format(name) if re.match(pattern=r'^\d', string=name) else name

    # if name.startswith('\\052'):
    #     name = name.replace('\\052', 'star')
    name = name.replace('\\052', 'star') if name.startswith('\\052') else name
...
def set_RecordName(name):
    ...
    # else:
    #     recordName = name
    name = name.replace('\\052', '*') if name.startswith('\\052') else name

    # if recordName.endswith('.'):
    #     recordName = recordName[0:-1]
    recordName = recordName[0:-1] if recordName.endswith('.') else recordName
...
def removeDotFromEnd(value):
    ...
    #     value=value[0:-1]
    # return value
    return value[0:-1] if value.endswith('.') else value


# def render_single_value_records(temp_path, zoneName, recordName, ttl, value, resource, aws_account_id):
def render_single_value_records(temp_path, zoneName, recordName, ttl, resource, aws_account_id, *values):
    ...
    with open(f'./{aws_account_id}/{zoneName}/{temp_path}.tf', 'a') as target:
        ...
        target.write(template.render(name=recordName, ttl=ttl, terrafromResource=resource, zone_id=zoneName, values=values))
...
def render_MX_records(temp_path, zoneName, recordName, ttl, resource, aws_account_id, *values_and_priorities): # a list of tuples
    ...
    with open(f'./{aws_account_id}/{zoneName}/MX.tf', 'a') as target:
        ...
        #         terrafromResource=resource, zone_id=zoneName))
        target.write(template.render(name=recordName, ttl=ttl, terrafromResource=resource, zone_id=zoneName, values_and_priorities=values_and_priorities)) # TODO - edit template render to get values as list of tuples


# def render_NS_records(temp_path, zoneName, recordName, ttl, resource, aws_account_id,
#     value1, value2="", value3="", value4=""):
def render_NS_records(temp_path, zoneName, recordName, ttl, resource, aws_account_id, *values):
    ...
    with open(f'./{aws_account_id}/{zoneName}/NS.tf', 'a') as target:
        ...
        # value4=value4,
        # terrafromResource=resource, zone_id=zoneName))
        target.write(template.render(name=recordName, ttl=ttl, terrafromResource=resource, zone_id=zoneName, values=values)) # TODO - edit template render to get values as list
...
#     value10=""):
def render_TXT_records(temp_path, zoneName, recordName, ttl, resource, aws_account_id, *values):
...
def parse_zone(zone, rs, aws_account_id):
    for record in rs['ResourceRecordSets']:
        ...
        record_func = locals().get(record['Type'].lower(), None) # locals() is a dict of top level declarations in the current file

        if record_func:
            record_func(set_ZoneName(zone), record, aws_account_id)
